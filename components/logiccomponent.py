...
from components import linegraphcomponent
    ...
    if not data_csv.empty:
        ...
        with tabs_component_data[0]:
            with tab1_graph_component[0]:
                linegraphcomponent.line_graph_component(data_csv[["created_at", "field1"]].copy(deep=True),
                                    "field1",
                                    "Temperature")
            with tab1_graph_component[1]:
                linegraphcomponent.line_graph_component(data_csv[["created_at", "field2"]].copy(deep=True),
                                    "field2",
                                    "Humidity")
        with tabs_component_data[1]:
            with tab2_graph_component[0]:
                linegraphcomponent.line_graph_component(data_csv[["created_at", "field3"]].copy(deep=True),
                                    "field3",
                                    "Air Quality")
